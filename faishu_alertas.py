...
# O bot vai buscar o Open ID automaticamente pelo e-mail
...
# ── E-mail (backup) ───────────────────────────────────────
USAR_EMAIL      = False   # mude para True se quiser manter e-mail também
...
def _enviar_webhook(url: str, card: dict):
    """Envia card via webhook Feishu (grupos externos)."""
    ...
    if data.get("code") != 0:
        ...
        log.info(f"✅ Webhook enviado com sucesso")
    return data
...
def enviar_ciclo(status: list[dict], novos: list[dict],
    ...
    agora = datetime.now().strftime("%d/%m/%Y %H:%M")
    alvos = destinos if destinos is not None and len(destinos) > 0 else [EMAIL_TESTE_FEISHU]
...
_EMOJI_LACRE = {"Coleta PA": "🟣", "Coleta Base": "🟢", "Devolução": "🟠", "Secundária": "🔴"}

def _card_lacre(lacres: list[dict]) -> dict:
    """Card de alerta de lacre para Coleta PA e Coleta Base — dar baixa no ID."""
    agora    = datetime.now().strftime("%d/%m/%Y %H:%M")
    elements = []
    elements.append({"tag": "div", "text": {"tag": "lark_md",
        "content": "**🔒 Viagens com Lacre — aguardando baixa no ID**\n"
                   "As viagens abaixo já receberam o lacre. Realize a baixa no sistema."}})
    elements.append({"tag": "hr"})
    for l in lacres:
        tel   = l.get("telefone", "")
        tel_txt = f"📞 {tel}" if tel and tel not in ("", "nan") else "📞 —"
        cat   = l.get("categoria", "Coleta Base")
        emoji = _EMOJI_LACRE.get(cat, "🟢")
        elements.append({"tag": "div", "text": {"tag": "lark_md", "content": (
            f"{emoji} **{cat}** | **ID:** {l.get('id','')}　|　🕐 Lacre: **{l.get('hora_lacre','')}**\n"
            f"🚛 **{l.get('viagem','')}**\n"
            f"📍 {l.get('origem','')} → {l.get('destino','')}\n"
            f"👤 {l.get('condutor','—')}　{tel_txt}"
        )}})
        elements.append({"tag": "hr"})
    elements.append({"tag": "note", "elements": [
        {"tag": "plain_text", "content": f"BRDrive Robot — JT Express | {agora}"}
    ...
    return {
        "config": {"wide_screen_mode": True},
        "header": {"title": {"tag": "plain_text",
                              "content": f"🔒 Lacre — {len(lacres)} viagem(ns) aguardando baixa"},
                   "template": "green"},
        "elements": elements,
    }


def enviar_lacre(lacres: list[dict], destinos: list[str]):
    """Envia alerta de lacres para o grupo de monitoramento."""
    if not lacres or not USAR_FEISHU:
        return
    card = _card_lacre(lacres)
    try:
        token = None
        ...
        log.error(f"❌ Feishu lacre falhou: {e}")


def enviar_alerta_sistema(mensagem: str, destinos: list[str]):
    """Envia alerta de erro/sistema (token expirado, falha de extração, etc.)."""
    if not USAR_FEISHU:
        return
    agora = datetime.now().strftime("%d/%m/%Y %H:%M")
    card = {
        "config": {"wide_screen_mode": True},
        "header": {"title": {"tag": "plain_text", "content": "⚠️ BRDrive Robot — Atenção"},
                   "template": "yellow"},
        "elements": [
            {"tag": "div", "text": {"tag": "lark_md", "content": mensagem}},
            {"tag": "note", "elements": [
                {"tag": "plain_text", "content": f"BRDrive Robot — JT Express | {agora}"}
            ]},
        ],
    ...
    try:
        ...
        log.error(f"❌ Feishu sistema falhou: {e}")
